...
def formula():
    query = input(
        ...
        "Type 0 or SHOW to see list of currencies available.\nPress Q to quit.\n"
    )
    if query == "Q" or query == "q":
        sys.exit()
    elif query == "SHOW" or query == "show" or query == "0":
        pprint(currencies)
        formula()
    else:
        curr, from_, to_ = query.split(" ")
        from_ = from_.upper()
        to_ = to_.upper()
        curr = float(round(int(curr), 2))
        amount = round(curr * fx[to_] / fx[from_], 2)
        print(f"{curr} of currency {from_} amounts to {amount} of currency {to_} today")
